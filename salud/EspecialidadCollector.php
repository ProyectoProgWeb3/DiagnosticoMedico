...
class EspecialidadCollector extends Collector
{
  ...
  function createespe($descripcion, $foto) {    
    $insertrow = self::$db->insertRow("INSERT INTO proyecto.especialidad(codespecialidad, descripcion, foto) VALUES (?, ?, ?)",  array( "{$descripcion}", "{$foto}"));
  }  
